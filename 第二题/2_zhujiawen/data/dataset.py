    ...
    if is_training:
        ...
        for idx,folder in enumerate(cate):
            for im in glob.glob(folder):
                ...
                #file_queue = tf.train.string_input_producer([im])  # 创建输入队列
                #image_reader = tf.WholeFileReader()
        ...
        #image = tf.cast(image_list, tf.float32)
        #label = tf.cast(label_list, tf.int32)
        #input_queue = tf.train.slice_input_producer([image,label], num_epochs=1)
        #image_contents = tf.read_file(input_queue[0])
        #image = tf.image.decode_jpg(image_contents)

        #image = my_preprocess_image(image, is_training)

        data,label=np.asarray(imgs,np.float32),np.asarray(labels,np.int32)
        #data = tf.cast(data, tf.float32)
        #data = my_preprocess_image(data, is_training)
        pass
